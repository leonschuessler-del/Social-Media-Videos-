/* comparison_split – Zwei-Panel-Vergleich im Visual-Science-Stil (Röntgen/Blueprint).
   Params:
     variant:   "vs" (Standard) | "then_now" | "myth_fact"
     heading:   optionale zentrale Überschrift
     left / right: { title, points: [string | {text, mark}], icon?, tag?, color?, caption? }
       icon: instantaneous_gear | progressive_gear | old_platform | modern_car | rope | belt | none
       Punkt-Präfixe: "+ " / "✓" → Haken, "✗" / "×" → Kreuz, "- " / "• " → Standardmarke der Variante
...
(function () {
  ...
  function normVariant(v) {
    ...
    if (/then|now|früher|frueher|heute|before|after|vorher|nachher|past|alt_neu|old_new/.test(k)) return "then_now";
    return "vs";
  }
  const ICONS = {};
  [["instantaneous_gear", "instantaneous instant instant_gear instantaneous_safety_gear sperrfang sperrfangvorrichtung"],
   ...
   ["belt", "belts riemen tragriemen flat_belt gurt"]].forEach(([k, al2]) => { ICONS[k] = k; al2.split(" ").forEach((a) => (ICONS[a] = k)); });
  ...
  function parsePoint(v) {
    ...
    s = s.trim();
    return s ? { text: s, mark, explicit: !!mark } : null;
  }
  const THEME = {
  ...
  function parsePanel(raw, side, th, P, variant, markMode) {
    ...
    tag = tag.toUpperCase().slice(0, 28);
    const ttl = title || (isL ? th.lt : th.rt);
    if (tag && ttl.toUpperCase() === tag) tag = "";
    ...
    const points = pts.map(parsePoint).filter(Boolean).slice(0, 8).map((pt) => {
      ...
      return { text: pt.text, mark: mk, mcol };
    });
    const tagMark = variant === "myth_fact" && tag ? (isL ? "cross" : "tick") : null;
    const caption = raw.caption != null ? clean(str(raw.caption)) : (CAPTION[icon] || "");
    return { side, title: ttl, points, icon, tag, tagMark, color, caption };
  }
  ...
  function layoutPanel(ctx, P, geo, fs, ih, maxLines) {
    let yy = geo.headerH;
    P.iconY = 0; P.ih = 0;
    if (P.icon !== "none" && ih > 0) { P.iconY = yy + 8; P.ih = ih; yy += ih + 34; }
    const lh = Math.round(fs * 1.28), gap = Math.round(fs * 0.66);
    const tw = geo.innerW - fs * 1.6, o = BODY_O(fs);
    P.rows = [];
    P.points.forEach((pt, i) => {
      let lines = wrapBalanced(ctx, pt.text, tw, o);
      if (maxLines) lines = clampLines(ctx, lines, maxLines, tw, o);
      const h = (lines.length - 1) * lh + fs * 1.18;
      P.rows.push({ i, lines, widths: lines.map((l) => L.measure(ctx, l, o)), y: yy, h, mark: pt.mark, mcol: pt.mcol, k: 0 });
      yy += h + gap;
    });
  ...
  function buildModel(ctx, p) {
    ...
    const variant = normVariant(P.variant ?? P.mode ?? P.style ?? P.kind);
    ...
    const m = { W, H, variant };
    m.left = parsePanel(rawL, "left", th, P, variant, markMode);
    m.right = parsePanel(rawR, "right", th, P, variant, markMode);
    m.heading = clean(str(P.heading ?? P.headline ?? P.title ?? P.ueberschrift ?? "")).toUpperCase();
    m.text = clean(str(p.text));
    m.badgeText = (clean(str(P.badge ?? P.vsLabel ?? "")) || "VS").toUpperCase().slice(0, 5);
    ...
    let top = sb.yMax(90);
    if (m.text) { m.tb = layoutTextBox(ctx, m.text); top = sb.yMax(96 + m.tb.h + 22); }
    if (m.heading) {
      ...
      hd.top = Math.max(top, sb.yMax(m.text ? 0 : 150)); hd.lh = hd.size * 1.15;
      m.hd = hd;
      top = hd.top + hd.lines.length * hd.lh + 22;
    }
    top = Math.max(top, sb.yMax(170));
    const avail = YB - top;
    ...
    let ts = 30, tl = null;
    for (const sz of [44, 40, 36, 32]) {
      const a = wrapBalanced(ctx, m.left.title, innerW, TITLE_O(sz)), b = wrapBalanced(ctx, m.right.title, innerW, TITLE_O(sz));
      if (a.length <= 2 && b.length <= 2 && !a.broken && !b.broken) { ts = sz; tl = [a, b]; break; }
    }
    if (!tl) { ts = 30; tl = [clampLines(ctx, wrapLines(ctx, m.left.title, innerW, TITLE_O(30)), 3, innerW, TITLE_O(30)), clampLines(ctx, wrapLines(ctx, m.right.title, innerW, TITLE_O(30)), 3, innerW, TITLE_O(30))]; }
    m.left.titleLines = tl[0]; m.right.titleLines = tl[1];
    const hasTag = !!(m.left.tag || m.right.tag);
    let hy = 28; const chipY = hy; if (hasTag) hy += 34 + 16;
    const titleTop = hy; const nT = Math.max(tl[0].length, tl[1].length);
    hy += nT * ts * 1.12; const ulY = hy + 6; hy = ulY + 22;
    const geo = { padX, innerW, headerH: hy, chipY, titleTop, ulY, ts, hasTag, nT };
    m.geo = geo;
    const anyIcon = m.left.icon !== "none" || m.right.icon !== "none";
    const cands = anyIcon
      ? [[256, 32], [236, 32], [216, 31], [196, 31], [196, 29], [176, 29], [176, 27], [156, 27], [156, 25], [136, 25], [136, 23], [116, 23], [116, 21], [0, 23], [0, 21], [0, 19]]
      : [[0, 32], [0, 30], [0, 28], [0, 26], [0, 24], [0, 22], [0, 20], [0, 19]];
    let ok = false;
    for (const c of cands) {
      const hl = layoutPanel(ctx, m.left, geo, c[1], c[0], 0), hr = layoutPanel(ctx, m.right, geo, c[1], c[0], 0);
      if (Math.max(hl, hr) <= avail) { ok = true; break; }
    }
    ...
    m.py = Math.round(top + Math.max(0, avail - m.ph) * 0.32);
  ...
  function timings(m, d) {
    const k = L.clamp(d / 8, 0.5, 1.4);
    const tm = { k, d };
    tm.panelD = 1.0 * k;
    tm.div0 = 0.15 * k; tm.divD = 0.9 * k;
    tm.badge0 = 0.5 * k; tm.badgeD = 0.7 * k;
    tm.head0 = 0.4 * k; tm.headD = 0.7 * k;
    tm.icon0 = 0.7 * k; tm.iconD = 0.9 * k;
    tm.b0 = 1.2 * k;
    const n = Math.max(1, m.nOrder);
    tm.b1 = Math.min(Math.max(tm.b0 + 0.22 * n, 0.56 * d), 0.8 * d);
    tm.step = (tm.b1 - tm.b0) / n;
    tm.bD = L.clamp(tm.step * 1.3, 0.3, 0.75);
    tm.act0 = tm.icon0 + tm.iconD * 0.5;
    tm.actD = L.clamp(0.32 * d, 1.2, 4.2);
    tm.liftD = 0.2 * tm.actD;
    tm.tE = tm.act0 + 0.42 * tm.actD;
    tm.tsInst = 0.08; tm.tsProg = 0.42 * tm.actD;
    tm.strike0 = tm.b1 + 0.1 * k;
    tm.hi0 = tm.b1 + 0.25 * k;
    return tm;
  }
  ...
  function drawFrame(ctx, bx, by, bw, bh, col, caption, t) {
    ...
    const cap = caption ? caption.toUpperCase() : "";
  ...
  function gearDynamic(c, g, col, t, tm, prog) {
    ...
    if (done > 0) {
      ...
      text(c, prog ? "Verzögerung Ø 0,2–1 g" : "Stopp schlagartig", x1, y0 + 8, { font: FONT.mono, size: 14, weight: 700, color: "rgba(226,240,255,0.95)", align: "right", alpha: done });
    }
  }
  function drawGear(ctx, m, key, bx, by, bw, bh, col, t, tm, prog) {
    const g = gearGeo(bx, by, bw, bh, prog);
    staticPart(ctx, m, key, bx, by, bw, bh, (c) => gearStatic(c, g, col, prog));
    gearDynamic(ctx, g, col, t, tm, prog);
  }
  ...
  function drawIcon(ctx, m, P, side, x, y, w, h, t, tm) {
    ...
    if (P.icon === "instantaneous_gear" || P.icon === "progressive_gear") {
      ...
      if (t >= settle) { // Endzustand ist statisch -> einmal rendern, danach blitten
        const lay = getLayer(m, key + "_end_" + tm.d.toFixed(3), w + 48, h + 48, (c2) => { c2.translate(24 - x, 24 - y); drawGear(c2, null, key, x, y, w, h, col, settle, tm, prog); });
        if (lay) { blit(ctx, lay, x - 24, y - 24, 1); return; }
      }
      drawGear(ctx, m, key, x, y, w, h, col, t, tm, prog);
      return;
    }
    switch (P.icon) {
  ...
  function drawHeading(ctx, m, tm, t) {
    ...
    const yl = hd.top + hd.size * 0.5 + (hd.lines.length - 1) * hd.lh / 2, half = hd.w0 / 2 + 28, len = 64 * L.easeOut(L.seg(t, 0.35 * tm.k, 0.8 * tm.k));
    if (len > 1) {
  ...
  function drawPanel(ctx, m, tm, t, side) {
    const P = m[side], isL = side === "left", k = tm.k, geo = m.geo;
    const e = L.easeOutExpo(L.seg(t, isL ? 0 : 0.08 * k, tm.panelD));
    if (e <= 0.001) return;
    ...
    ctx.save();
    try {
      ctx.translate(Math.round((isL ? -1 : 1) * (1 - e) * (w + 220)), 0);
      GA = baseA;
      const scaled = VEC;
      ...
      if (P.ih > 0) {
        ...
        const rv = L.easeInOut(L.seg(t, tm.icon0 + (isL ? 0 : 0.12 * k), tm.iconD));
        if (rv > 0) {
          ctx.save();
          try {
            ...
            drawIcon(ctx, m, P, side, ix, iy, iw, ihh, t, tm);
      ...
      if (P.rows.length) {
        ...
        const maxK = Math.max.apply(null, P.rows.map((r) => r.k));
        const strikeSide = m.strikeSide === side;
        const doneT = Math.max(tm.b0 + tm.step * maxK + tm.bD, strikeSide ? tm.strike0 + (nR - 1) * 0.14 * k + 0.5 * k : 0) + 0.02;
        const paintRow = (c, row, pr, struck, tt, useM) => {
          const ep = L.easeOut(pr), ry = y + row.y;
          ...
          row.lines.forEach((ln, j) => text(c, ln, tx0 + (1 - ep) * 26, ry + fs * 0.9 + j * P.lh, { size: fs, weight: 600, font: FONT.body, color: C.white, alpha: ep * (1 - 0.3 * struck) }));
        ...
        if (t >= doneT && !scaled) {
          ...
          const lay = getLayer(m, "rows_" + side, w + 20, RH, (c2) => { c2.translate(10 - x, -RY0); P.rows.forEach((row) => paintRow(c2, row, 1, strikeSide ? 1 : 0, 0, m)); });
          if (lay) { blit(ctx, lay, x - 10, RY0, 1); cached = true; }
        }
        if (!cached) P.rows.forEach((row) => {
          const pr = L.seg(t, tm.b0 + tm.step * row.k, tm.bD);
          if (pr <= 0) return;
          const struck = strikeSide ? L.easeInOut(L.seg(t, tm.strike0 + row.i * 0.14 * k, 0.5 * k)) : 0;
          paintRow(ctx, row, pr, struck, t, m);
        });
      }
    } finally {
      GA = 1; ctx.restore();
